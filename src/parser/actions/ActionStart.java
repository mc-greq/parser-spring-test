...
import parser.*;
import parser.logicclasses.MyExceptionHandler;
import parser.logicclasses.MyHandler;
import parser.logicclasses.Unzipper;
import parser.properties.PropertiesHolder;
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        new Thread(() -> {
            ...
            try {
                ...
                FileWriter outSFirma = new FileWriter(propertiesHolder.getFileFirma());
                // otwieram strumień zapisu dla uprawnień
                FileWriter outSUprawnienie = new FileWriter(propertiesHolder.getFileUprawnienia());
                // otwieram strumien zapisu dla oddziałów
                FileWriter outSOddzial = new FileWriter(propertiesHolder.getFileOddzialy());
                // otwieram strumien zapisu dla firm bez adresu calosc_calosc_ExtendedAddress_.xml
                FileWriter outExAddress = new FileWriter(propertiesHolder.getFileNoAddress());
                // otwieram strumiem zapisu kodów teryt do pliku
                FileWriter outTertyt = new FileWriter(propertiesHolder.getFileTeryt());
...
//            MyHandler handler = new MyHandler(outSFirma, outSUprawnienie, outSOddzial);
                ...
                if(!testExAddress){
                    Files.deleteIfExists(new File(propertiesHolder.getFileNoAddress()).toPath());
                }
